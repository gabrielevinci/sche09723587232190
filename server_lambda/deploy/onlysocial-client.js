 ...
 */
async function _uploadVideoToOnlySocial({ videoUrl, filename }) {
...
async function _createOnlySocialPost({ accountId, mediaId, caption, postType, scheduledFor }) {
    ...
    console.log(`📝 [OnlySocial] Creating post for account ID ${accountId}`);
    console.log(`   Date: ${date}, Time: ${time}, Media ID: ${mediaId}`);
    const payload = {
        accounts: [accountId], // ✅ FIX: Account ID deve essere nell'array
        versions: [{
                account_id: 0, // ✅ FIX: 0 = usa il primo account dall'array
                is_original: true,
                content: [{
                        body: caption,
                        media: [mediaId],
 ...
 */
async function _scheduleOnlySocialPost({ postUuid, postNow = false }) {
